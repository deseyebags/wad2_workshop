            ...
            <div class="card-body rounded-3" style="background-color: ">
              ...
              <div v-else>
                ...
                <div class="row">
                  <div class="col-6">
                    <button
                      @click="updatePost(editingPost)"
                      class="btn btn-pri me-2 w-100 m-3"
                    >
                      Save
                    </button>
                  </div>
                  <div class="col-6">
                    <button @click="cancelEdit" class="btn btn-sec w-75 m-3">
                      Cancel
                    </button>
                  </div>
                </div>
        ...
        <form @submit.prevent="addPost" class="p-4 rounded">
          <div class="form-group col-md-6 mx-auto pb-2 w-100">
            <input
              ...
              class="form-control form-control-lg border-0 shadow-sm w-100 rounded"
              placeholder="title"
            />
          </div>
  ...
  methods: {
    ...
    async addPost() {
      const response = await axios.post(
        "http://workshop50.bchwy.com/1create.php",
        this.newPost,
        {
          headers: {
    ...
    async deletePost(id) {
      await axios.post(
        "http://workshop50.bchwy.com/4delete.php",
        { id },
        {
          headers: {
            "Content-Type": "application/x-www-form-urlencoded",
